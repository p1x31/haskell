...
-- printBoard(writeMove ("test", ((10,7), V)) (autoResize [[Just 't'],[Just 'e'],[Just 's'],[Just 't']]))
...
replenishRack xs
  ...
  | otherwise = return xs
--try 1
--replenishRack xs = LRand(replicate RackSize (evalRand newLetter))
...
allWords1 :: Dict -> Char -> [String]
allWords1 xs@(x:xs') c = [x | x<-xs', c `elem` x ]
-- Exercise, hard.
--
...
allWords2 :: Dict -> Char -> Int -> Int -> [(String, Int)]
allWords2 dict c i j = [(x,pos)|  x <-xs, pos <- elemIndices c x, checkSize x pos i j]
  where xs = allWords1 dict c
        checkSize x pos i j
          | length (take pos x) <= i && length (drop (pos+1) x) <= j = True
          | otherwise = False
...
allWords3 :: Dict -> Rack -> Char -> Int -> Int -> [(String, Int)]
allWords3 dict rack c i j = [ x | x <- xs, guard (fst x) rack]
  where xs = allWords2 dict c i j
        guard y rack
          | y \\ (rack ++ [c]) == [] = True
          | otherwise = False
...
greedyBestMove :: Dict -> Rack -> Board -> FullMove
greedyBestMove dict rack b = maximumBy (compare `on` moveScore ) (allMoves dict rack b)
